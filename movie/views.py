...
def home(request):
    ...
    #return render(request, 'home.html', {'name': 'Alessandro Soccol Mejia'})
    searchTerm = request.GET.get('searchMovie')
    if searchTerm:
        movies = Movie.objects.filter(title__icontains=searchTerm)
    else:
        movies = Movie.objects.all()
    return render(request,'home.html', {'searchTerm':searchTerm, 'movies':movies, 'name': 'Alessandro Soccol Mejia'})
...
def statistics_view(request):
    ...
    for movie in all_movies:
        if movie.genre:
            # Toma solo el primer género si hay varios separados por coma
    ...
    genre_image_png = genre_buffer.getvalue()
    genre_buffer.close()

    genre_graphic = base64.b64encode(genre_image_png)
    genre_graphic = genre_graphic.decode('utf-8')

    # Renderizar la plantilla statistics.html con la gráfica
    return render(
        request,
        'statistics.html',
        {'graphic': graphic, 'genre_graphic': genre_graphic}
    )